# Installing iOS Senior Agent Framework
...
```bash
git clone https://github.com/user/ios-senior-agent.git ~/.ios-senior-agent
```
...
From the user's iOS project root directory, symlink the `skills/` directory and the agent config into the project so the AI tool can discover them within its workspace scope.

What to symlink:
- `~/.ios-senior-agent/skills` → into the project (so skills are discoverable)
- `~/.ios-senior-agent/agents/ios-senior-agent.md` → into the project's tool-specific config file

**Examples:**
```bash
# Symlink skills into project
ln -sf ~/.ios-senior-agent/skills ./skills

# Symlink agent config (adapt the target filename to your tool's convention)
ln -sf ~/.ios-senior-agent/agents/ios-senior-agent.md ./CLAUDE.md       # Claude Code
ln -sf ~/.ios-senior-agent/agents/ios-senior-agent.md ./GEMINI.md       # Antigravity / Gemini CLI
ln -sf ~/.ios-senior-agent/agents/ios-senior-agent.md ./AGENTS.md       # Codex
```
...
You should see symlinks pointing back to `~/.ios-senior-agent/`.

## Updating

```bash
cd ~/.ios-senior-agent && git pull
```
...
2. **Optionally delete the global clone:**
   ```bash
   rm -rf ~/.ios-senior-agent
   ```

## What Gets Installed

```
~/.ios-senior-agent/                     # Global clone (shared by all projects)
├── skills/                              # 16 domain-specific iOS skills
│   ├── using-ios-senior-skills/         # Meta-skill: orchestrates all others
...
├── agents/
│   └── ios-senior-agent.md              # Agent definition
└── docs/

your-ios-project/                        # Any iOS project
├── skills → ~/.ios-senior-agent/skills  # Symlink (gitignored)
├── CLAUDE.md → ~/.ios-senior-agent/...  # Symlink (gitignored)
└── ...your project files
```
